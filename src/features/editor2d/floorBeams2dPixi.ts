...
import type { FloorBeamOverStockPixiPaint } from "./floorBeamOverStock2dTheme";
...
export interface DrawFloorBeams2dOptions {
  ...
  /** Цвета из theme.css; если задано вместе с highlightOverLinearStock — без hardcoded fallback. */
  readonly overStockPaint?: FloorBeamOverStockPixiPaint | null;
}
...
export function drawFloorBeams2d(
  ...
  const stockPaint = opts?.overStockPaint ?? null;
  ...
  for (const beam of beams) {
    ...
    const { stroke: baseStroke, fill: baseFill } = strokeAndFillForBeam(beam, project);
    const sel = selectedIds.has(beam.id);
    const overStock = showOver && stockPaint != null && floorBeamExceedsLinearStockLength(project, beam);

    let fillColor = baseFill;
    let fillA = alphaFill;
    let strokeColor = sel ? BEAM_SELECTED : baseStroke;
    let strokeA = sel ? 0.95 : alphaStroke;
    let strokeW = sel ? 2 : 1;

    if (overStock) {
      fillColor = stockPaint!.fillRgb;
      fillA =
        appearance === "context" ? stockPaint!.fillAlphaContext : stockPaint!.fillAlphaActive;
      if (sel) {
        strokeColor = BEAM_SELECTED;
        strokeA = 0.95;
        strokeW = 2;
      } else {
        strokeColor = stockPaint!.strokeRgb;
        strokeA =
          appearance === "context" ? stockPaint!.strokeAlphaContext : stockPaint!.strokeAlphaActive;
        strokeW = 1;
      }
    }
